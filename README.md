...
| **AI Intelligence** | "Hey Firehouse" voice assistant, AI shift briefings, staffing forecasts + burnout detection, NFPA training-gap analysis, auto vacancy fill. Ask Open Firehouse is the in-app ops face of the same department-agent plug (Duty/Board chat plus a weekday morning shift brief routine). A department-local MCP exposes a small set of fire verbs (read incidents/roster/duty/board/training/apparatus; queue NERIS submit, notify-chief, and unit clear for a chief to accept on the Dashboard). |
