...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ data: row });
  } catch (e) { res.status(500).json({ error: 'Failed to fetch asset' }); }
});

router.post('/', async (req, res) => {
  try {
    if (!req.body.name) return res.status(400).json({ error: 'name is required' });
    const { id: _ignore, ...body } = req.body; // strip any client-generated id
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    if (!await db.findById(id, req.user.department_id)) return res.status(404).json({ error: 'Asset not found' });
