#!/usr/bin/env node
/**
 * Self-Heal Agent — OpenFirehouse
 *
 * Constrained agent loop that runs inside GitHub Actions.
 * Receives a bug diagnosis and attempts a surgical fix.
 *
...
const tools = [
  {
    name: 'read_file',
    description: 'Read the contents of a file at the given path (relative to repo root).',    input_schema: {
      type: 'object',
  ...
  {
    name: 'edit_file',
    description: 'Replace old_string with new_string in the file. The old_string must be unique.',    input_schema: {
      type: 'object',
      properties: {
        path: { type: 'string' },
  ...
  {
    name: 'write_file',
    description: 'Write content to a new file. Fails if the file already exists (use edit_file).',
    input_schema: {
      type: 'object',
  ...
  {
    name: 'finish',    description: 'Terminate the agent loop. Call this when done.',
    input_schema: {
      type: 'object',
      properties: {
...
function executeTool(name, input) {
  switch (name) {
    ...
    case 'list_directory': {
      ...
      try {
        const result = execSync(`grep -rn "${input.pattern.replace(/"/g, '\\"')}" ${input.path || '.'}`, {
          cwd: REPO_ROOT, encoding: 'utf-8', maxBuffer: 1024 * 1024, timeout: 10000,
        });
    ...
    case 'write_file': {
      ...
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });      fs.writeFileSync(fullPath, input.content);
      return { success: true };
    }
    case 'bash': {
      ...
      try {
        const result = execSync(input.command, {
          cwd: REPO_ROOT, encoding: 'utf-8', maxBuffer: 1024 * 1024, timeout: 60000,
        });
...
Your job: verify the diagnosis against the actual code, apply a surgical fix if confirmed, and call finish. If you cannot confirm the diagnosis, call finish with applied_fix: false. Do NOT produce speculative changes. Prefer minimal, targeted edits.`;
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    for (const block of response.content) {
      if (block.type === 'tool_use') {        const result = executeTool(block.name, block.input);
