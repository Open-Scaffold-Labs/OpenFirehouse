...
function ApparatusTimesPanel({ incidentId }) {
  ...
  return (
    <>
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Apparatus Times (NFIRS)</p>
      {times === null ? (
        <p className="text-gray-400 text-xs">Loading…</p>
      ) : !times.length ? (
        <p className="text-gray-400 text-xs">No per-apparatus times recorded for this incident.</p>
      ) : (
...
const TYPE_COLORS = {
  ...
  'False Alarm':          'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 ring-gray-400/20',
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <button
                          ...
                          className="p-0.5"
                        >
                          <ChevronRight className={`h-4 w-4 text-gray-400 transition-transform ${expandedId === inc.id ? 'rotate-90' : ''}`} />
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-3.5" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setTimelineIncident(inc)}
                            ...
                            className="rounded-lg p-1.5 text-gray-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/50 transition-colors" title="View timeline">
                          ...
                          {navigator.share && (
                            <button onClick={() => {
                              ...
                              className="rounded-lg p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 transition-colors" title="Share incident">
                              <Share2 className="h-4 w-4" />
                            </button>
                          )}
                          <button onClick={() => { setEditingIncident(inc); setFormOpen(true); }}
                            aria-label="Edit incident"
                            className="rounded-lg p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/50 transition-colors" title="Edit">
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button onClick={() => setDeletingIncident({ ...inc, name: inc.incidentNumber })}
                            aria-label="Delete incident"
                            className="rounded-lg p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/50 transition-colors" title="Delete">
                      ...
                      <tr key={`${inc.id}-detail`} className="bg-gray-50 dark:bg-gray-950">
                        <td colSpan={9} className="px-8 py-4">
                          <div className="grid grid-cols-1 sm:grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                            <div>
                              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Units Responding</p>
                              {inc.units.length ? (
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Personnel ({inc.personnel.length})</p>
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Injuries</p>
                            ...
                            {inc.notes && (
                              <div className="col-span-2 sm:col-span-3">
                                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Narrative</p>
